      ...
      <div class="container-fluid table-responsive">  
        ...
        <table class="table table-striped table-bordered" style="display: table;">
          <thead>
            ...
            <th class="text-center"><h5 class="h5-responsive"><b>No applications</b></h5></th>
          </thead>
          <tbody>
            <?php 
              $sql = "SELECT * FROM internships WHERE deleted = 0 AND emp_id = $emp_id";
            ...
            <?php while($internship = mysqli_fetch_assoc($internships)):?>
              <?php $applications_q = "SELECT COUNT(*) as total FROM `applications` WHERE int_id = {$internship['id']}"; 
                    $applications = $db->query($applications_q); 
                    if ($applications) {  
                      $result =mysqli_fetch_assoc($applications) ;  
                      $total_count = $result['total']; // Get the count  
                     } else {  
                      // Handle error, e.g., log it or display a message  
                      $total_count = 0; // Or some error handling  
                  }  
                     ?>
              <tr>
                <td>
